...
func main() {
	...
	flag.StringVar(&userLogPath, "user-output", "users.json", "Path to output users log")
	flag.StringVar(&activityLogPath, "activity-log-path", "activity.json", "File to output activity log")
	...
	if noauth {
		...
		config.PasswordCallback = func(c ssh.ConnMetadata, pass []byte) (*ssh.Permissions, error) {
			...
			logLoginAttempt(c.RemoteAddr().String(), c.User(), string(pass))
